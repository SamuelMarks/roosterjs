...
export const IndentWhenTab: ContentEditFeature = {
    keys: [KEY_TAB],
    shouldHandleEvent: (event, editor) => !(event.rawEvent as KeyboardEvent).shiftKey && cacheGetListElement(event, editor),
...
export const MergeInNewLine: ContentEditFeature = {
    ...
    handleEvent: (event, editor) => {
        ...
        if (li.previousSibling) {
            editor.runAsync(() => {
                let br = fromHtml('<BR>', editor.getDocument())[0];
    ...
    handleEvent: (event, editor) => {
        editor.runAsync(() => {
            editor.performAutoComplete(() => {
                let cursorData = cacheGetCursorEventData(null /*pass null for event, force get fresh CursorData*/, editor);
                let textBeforeCursor = cursorData.getXCharsBeforeCursor(3);
